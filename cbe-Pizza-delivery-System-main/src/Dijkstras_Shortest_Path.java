...
class Dijkstras_Shortest_Path
{
    ...
    public Dijkstras_Shortest_Path(int nodes)  // constructor
    {
        ...
        pathSequence = new List[number_of_nodes + 1];
        previousNode = new int[number_of_nodes + 1];
        for (int i = 0; i <= number_of_nodes; i++) {
            pathSequence[i] = new ArrayList<>();
        }
    ...
    public List<Integer> getPathSequence(int destinationNode) {
        List<Integer> path = new ArrayList<>(pathSequence[destinationNode]);
            ...
            if (!settled.contains(destinationNode))  
            {
                if (adjacencyMatrix[evaluationNode][destinationNode] != Integer.MAX_VALUE)
                {
                    ...
                    if (newDist < distances[destinationNode])
                    {
                        ...
                        previousNode[destinationNode] = evaluationNode; // Store the previous node
                        pathSequence[destinationNode].clear();
                        pathSequence[destinationNode].addAll(pathSequence[evaluationNode]);
                        pathSequence[destinationNode].add(evaluationNode);
                    }
                    }
